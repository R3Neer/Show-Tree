...
let expected_columns = [path type size]
...
assert ($json | str contains '"path"') 'Structured output should serialize its public rows directly to JSON.'
...
assert (not ($json | str contains '"name"')) 'The redundant basename column must not bloat the public contract.'

let table_text = ($with_files | table | ansi strip)
for header in [path type size] {
    assert ($table_text | str contains $header) $"Explicit table output should expose the ($header) column at ordinary terminal widths."
}
